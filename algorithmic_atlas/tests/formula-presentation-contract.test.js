...
test("formula presentation policy is a closed reviewed snapshot", () => {
  ...
  assert.equal(policy.snapshot.publishedChapterCount, 56);
  assert.equal(policy.snapshot.displayFormulaTotal, 516);
  assert.equal(policy.snapshot.basicFormulaTotal, 35);
  assert.equal(policy.snapshot.interactiveFormulaTotal, 481);
  assert.equal(
