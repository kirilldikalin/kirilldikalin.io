  ...
  runtime.boot("matrices-linear-systems-stability", function (root) {
    ...
    function drawConditioning(svg, frame) {
      ...
      values.forEach(function (entry, index) {
        const y = 130 + index * 82, log = Math.log10(Math.max(entry[1], 1e-18));
        const width = Math.max(3, Math.min(330, (log + 18) / 20 * 330));
        ...
        visual.append(svg, "rect", { x: 500, y: y + 14, width: width, height: 22, class: index === 1 ? "numeric-cell is-active" : "numeric-matrix-cell", "data-visual-part": "error-bar", "data-error-kind": entry[0] });
        visual.text(svg, 842, y + 31, entry[1].toExponential(3), "numeric-cell-label", "end");
      });
      visual.text(svg, 500, 490, "Шкала логарифмическая: длина показывает порядок величины", "is-muted");
    }
    function drawStrassen(svg, frame, state) {
      const matrix = frame.result || state.inputs.matrix;
      const cell = Math.min(62, 260 / matrix.length), x = 70, y = 130;
      visual.text(svg, 55, 70, "Четыре квадранта и семь рекурсивных произведений", "is-strong");
      visual.drawMatrix(svg, matrix, { x: x, y: y, cell: cell, active: [] });
      const splitX = x + matrix[0].length * cell / 2, splitY = y + matrix.length * cell / 2;
      visual.append(svg, "line", { x1: splitX, y1: y, x2: splitX, y2: y + matrix.length * cell, class: "numeric-plot-line" });
      visual.append(svg, "line", { x1: x, y1: splitY, x2: x + matrix[0].length * cell, y2: splitY, class: "numeric-plot-line" });
      [["11", x + 10, y + 22], ["12", splitX + 10, y + 22], ["21", x + 10, splitY + 22], ["22", splitX + 10, splitY + 22]].forEach(function (label) { visual.text(svg, label[1], label[2], label[0], "is-strong"); });
      ...
      visual.text(svg, 70, 465, "глубина " + frame.depth + " · блок " + frame.size + "×" + frame.size + " · путь " + (frame.path || "A²"), "is-muted");
      visual.text(svg, 70, 505, frame.phase === "split" ? "Блок делится; далее вычисляются M1…M7" : frame.phase === "base" ? "Базовый блок умножается обычным способом" : "Произведения собираются обратно в C11…C22", "is-strong");
    }
    function render(state) {
      const frame = state.current; visual.clear(figure.svg, "Матричная трасса", frame.message);
      if (frame.mode === "strassen") drawStrassen(figure.svg, frame, state);
