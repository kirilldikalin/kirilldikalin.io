(function (root, factory) {
  ...
  function multiplyMatrices(left, right) {
    ...
    for (let i = 0; i < a.length; i += 1) for (let k = 0; k < b.length; k += 1) for (let j = 0; j < b[0].length; j += 1) result[i][j] += a[i][k] * b[k][j];
    return result;
  }
  function addMatrices(a, b, sign) { return a.map(function (row, i) { return row.map(function (value, j) { return value + (sign || 1) * b[i][j]; }); }); }
  ...
  function gaussianFrames(rawMatrix, rawVector, pivoting) {
    ...
    for (let column = 0; column < n; column += 1) {
      ...
      if (Math.abs(augmented[pivot][column]) <= EPSILON) throw new RangeError("Матрица вырождена или численно неотличима от вырожденной.");
    ...
    for (let row = n - 1; row >= 0; row -= 1) { let rhs = augmented[row][n]; for (let column = row + 1; column < n; column += 1) rhs -= augmented[row][column] * solution[column]; solution[row] = rhs / augmented[row][row]; frames.push({ mode: "gaussian", phase: "back-substitute", pivot: row, target: null, matrix: cloneMatrix(augmented), solution: solution.slice(), message: "Обратная подстановка определила x_" + (row + 1) + "." }); }
  ...
  function multiplyVector(matrix, vector) { return matrix.map(function (row) { return row.reduce(function (sum, value, index) { return sum + value * vector[index]; }, 0); }); }
  function residual(matrix, solution, vector) { return multiplyVector(matrix, solution).map(function (value, index) { return vector[index] - value; }); }
  function infinityNormVector(vector) { return Math.max.apply(null, vector.map(Math.abs)); }
  function infinityNormMatrix(matrix) { return Math.max.apply(null, matrix.map(function (row) { return row.reduce(function (sum, value) { return sum + Math.abs(value); }, 0); })); }
  ...
  function conditionInfinity(matrix) { const a = validateMatrix(matrix, true); return infinityNormMatrix(a) * infinityNormMatrix(inverse(a)); }
